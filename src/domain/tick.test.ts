...
const btc = Tick.makeScale("perp", 5); // rawTick 0.1
const spot = Tick.makeScale("spot", 2); // rawTick 1e-6
...
describe("Tick.parse", () => {
  ...
  it("does not round-trip through floats", () => {
    ...
    expect(parsed("40000.12345678", Tick.makeScale("spot", 0))).toBe(4000012345678);
  });
...
describe("Tick.format", () => {
  it("renders with exactly the scale's decimals", () => {
    ...
    expect(Tick.format(parsed("7", Tick.makeScale("perp", 6)), Tick.makeScale("perp", 6))).toBe("7");
  });
...
describe("Tick.makeScale", () => {
  it("derives decimals as D − szDecimals (6 perp, 8 spot)", () => {
    expect(Tick.makeScale("perp", 5).decimals).toBe(1);
    expect(Tick.makeScale("spot", 2).decimals).toBe(6);
    expect(Tick.makeScale("perp", 0).decimals).toBe(6);
  });
});
