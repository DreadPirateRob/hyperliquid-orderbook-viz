...
6. **Trade–level interaction**: per price `consumedTotal`, `cancelledTotal`, `hitCount`, `lastHitAt`; per side over W = 60 s `consumedRate` (coin/s) and `cancelRatio = cancelled / (consumed + cancelled)`. Fill markers fire on trade arrival, not on the diff.
...
10. **Book shape**: sparkline = cumulative curve per side (20 points); `convexity = depthWithin(5 bps) / depthWithin(20 bps)` per side.
